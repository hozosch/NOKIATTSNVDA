#!/usr/bin/env python3
"""Add Nokia 5500 Klatt paths exercised by long NVDA diagnostics."""
...
DISPATCH_ANCHOR = "    case 0xf845b080u: goto L_f845b080;\n"
DISPATCH_LINE = "    case 0xf845b082u: goto L_f845b082;\n"
BODY_ANCHOR = "L_f845b084:\n"
BODY = """L_f845b082:
    nokia_at=0xf845b082u;
...
TRACED_BRANCH = (
    "    if ((u_134800 & UINT64_C(0xff))) goto L_f845b082;\n"
    "    goto L_f845b088;\n"
)
...
def main() -> None:
    ...
    changed = False
    if "L_f845b082:" not in text:
        if text.count(DISPATCH_ANCHOR) != 1:
            raise SystemExit("5500 Klatt dispatch anchor is missing or ambiguous")
        if text.count(BODY_ANCHOR) != 1:
            raise SystemExit("5500 Klatt body anchor is missing or ambiguous")
    ...
    if UNTRACED_BRANCH in text:
        ...
        raise SystemExit("5500 Klatt regression branch is missing or ambiguous")

    if changed:
        args.source.write_text(text, encoding="utf-8")
        print("added Nokia 5500 Klatt regression path")
    else:
        print("Nokia 5500 Klatt regression path is already present")
