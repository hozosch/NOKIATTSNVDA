...
struct NokiaRuntime {
    ...
    uint8_t extend_initial_g_release;
...
/* At Nokia's neutral duration, an initial German /g/ keeps its high-front
   release target for only one 5 ms Klatt frame. At a slightly slower setting
   the same unmodified frontend naturally keeps it for two frames before the
   final 5 ms release frame. Reproduce only that timing at rate 50; do not
   touch the following vowel or any Klatt amplitude. This text/shape match is
   intentionally narrow for the diagnostic build and will be replaced by an
   acoustic phone match if the listening test confirms the cause. */
static int extend_initial_g_release_timing(
    ...
    const int16_t extension = 5;
    if (!r->extend_initial_g_release) return 1;
    ...
    durations[2] = (int16_t)(durations[2] + extension);
    if (pitch_time_count &&
        !prosody_array(r, pitch_time_address, pitch_time_count, &times))
        return 0;
    for (i = 0; i < pitch_time_count; ++i)
        if (times[i] >= old_release_end && times[i] <= 32767 - extension)
            times[i] = (int16_t)(times[i] + extension);
...
static int apply_prosody_rate(NokiaRuntime *r, int continuation) {
    ...
    if (factor > 0.999 && factor < 1.001 &&
        ...
        !r->extend_initial_g_release) return 1;
#endif
    ...
    if (!extend_initial_g_release_timing(
