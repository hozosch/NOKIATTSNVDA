...
struct NokiaRuntime {
    ...
    uint8_t lengthen_initial_g_release;
...
/* Test a clearly longer initial German /g/ release at Nokia's neutral rate:
   extend the high-front release target from its nominal 10 time units to 20,
   then move the following prosody points right by the same amount. Test52
   used only half this extension. The following vowel parameter values, Klatt
   amplitudes, other rates and non-initial G phones remain untouched. This
   text/shape match is intentionally narrow for the diagnostic build. */
static int lengthen_initial_g_release_timing(
    ...
    const int16_t extension = 10;
    if (!r->lengthen_initial_g_release) return 1;
...
static int apply_prosody_rate(NokiaRuntime *r, int continuation) {
    ...
    if (factor > 0.999 && factor < 1.001 &&
        ...
        !r->lengthen_initial_g_release) return 1;
#endif
    ...
    if (!lengthen_initial_g_release_timing(
